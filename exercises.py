...
Student Performance Analysis using Numpy
    Design a Python program to analyse the student performance data across multiple subjects. The
    program should allow users to get the input data for a specified number of students, including
    their names, ages, and scores in Math, Science, Physics, and Chemistry. Based on this data, the
  program will calculate and display various statistics:
...
def initialize_student_data(num_students):
 ...
 student_data.append([name, age, math_score, science_score, physics_score, chemistry_score]) # Append student data to list
...
def filter_students(student_data, min_age, min_score, subject='Math'):
 subject_index = {'Math': 2, 'Science': 3, 'Physics': 4, 'Chemistry': 5}[subject] # Determine index of subject based on input
...
-----Program 1b-----
  Develop a machine learning model that accurately classifies iris flowers into one of three species based on their
  sepal and petal measurements using pandas library
Objective:
  To build a Decision Tree classifier using the Iris dataset that achieves high accuracy in predicting the species of
  iris flowers.
The model should be able to:
  • Accurately Classify Iris Species: Develop a classifier that correctly identifies the species of iris
  flowers based on their sepal length, sepal width, petal length, and petal width.
  • Evaluate Model Performance: Measure the performance of the classifier using metrics such as
...
data = pd.DataFrame(cancer.data, columns=cancer.feature_names) # Converting the dataset to a pandas DataFrame
data['target'] = cancer.target # Adding the target column to the DataFrame
# Display a concise summary of the DataFrame
print(data.info()) # Displaying a concise summary of the DataFrame, including the number of entries, columns, non-null values, and data types
# Display the first few rows of the dataset
print(data.head(1)) # Displaying the first few rows of the DataFrame to get an initial look at the data
# Display basic statistics
print(data.describe()) # Displaying basic statistical details like mean, std deviation, min, and max values for each column
...
plt.plot(data.index, data['mean radius'], label='Mean Radius') # Creating a line plot for the 'mean radius' column
...
# Scatter Plot
plt.figure(figsize=(10, 6)) # Setting the figure size for the plot
plt.scatter(data['mean radius'], data['mean texture'], c=data['target'], cmap='coolwarm', alpha=0.5) # Creating a scatter plot with 'mean radius' and 'mean texture', color-coded by the target class
...
# Bar Plot
plt.figure(figsize=(10, 6)) # Setting the figure size for the plot
plt.bar(data['target'].value_counts().index, data['target'].value_counts().values) # Creating a bar plot for the target class distribution
...
# Histogram
plt.figure(figsize=(10, 6)) # Setting the figure size for the plot
plt.hist(data['mean area'], bins=30, alpha=0.7) # Creating a histogram for the 'mean area' column with 30 bins
...
labels=['Malignant', 'Benign']) # Creating a box plot for the 'mean radius' column, grouped by the target class
...
data = pd.DataFrame(cancer.data, columns=cancer.feature_names) # Create a DataFrame with feature names as columns
data['target'] = cancer.target # Add the target column to the DataFrame, which contains the class labels
# Display a concise summary of the DataFrame
print(data.info()) # Displaying a concise summary of the DataFrame, including the number of entries, columns, non-null values, and data types
# Display the first few rows of the dataset
print(data.head(1)) # Displaying the first few rows of the DataFrame to get an initial look at the data
...
sns.countplot(x='target', data=data, palette='coolwarm') # Create a count plot to visualize the number of Malignant vs. Benign cases
...
# Violin Plot
plt.figure(figsize=(10, 6)) # Set the size of the figure for the plot
sns.violinplot(x='target', y='mean radius', data=data, palette='coolwarm') # Create a violin plot for 'mean radius' by target class
...
hue='target', palette='coolwarm') # Create a pair plot for selected features, color-coded by target class
plt.title('Pair Plot') # Add a title to the plot
plt.show() # Display the plot
# Heatmap
plt.figure(figsize=(20, 20)) # Set the size of the figure for the plot
sns.heatmap(data.corr(), annot=True, fmt='.2f', cmap='coolwarm') # Create a heatmap for the correlation matrix of features
...
Problem Statement:
  ...
  problem is to develop a K-Nearest Neighbors (KNN) classifier using the original fourdimensional feature space and 
  compare its performance to a KNN classifier trained on a twodimensional feature space obtained through LDA. 
  By evaluating and comparing the accuracy of these models, the problem seeks to understand how LDA's dimensionality 
  reduction impacts the performance of KNN classification for Iris flower species.
-----Coding-----
...
feature names = iris.feature names # Get the names of the features
...
✗_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42) # Split the data, 70% for training and 30% for testing
...
print(pd.Data Frame(X_train, columns-feature_names).head(3)) # Convert the first 3 rows of the original feature data to a DataFrame and display it
# Apply LDA for dimensionality reduction
lda = LDA(n_components=2) # Initialize LDA with 2 components for dimensionality reduction
X_train_lda = lda.fit_transform(X train_scaled, y_train) # Fit LDA on the scaled training data and transform it
...
print(pd.DataFrame(X_train_Ida, columns=['LDA Component 1', 'LDA Component 2']).head(3)) # Convert the first 3 rows of the LDA-transformed data to a DataFrame and display it
...
print(f"Accuracy: {accuracy_original:.2f}") # Print the accuracy of the KNN model on the original features
...
knn lda.fit(X train lda, y train) # Train KNN model on the 2D LDA-transformed feature data
y_pred_lda = knn_lda.predict(X_test_lda) # Predict on the test set
accuracy_lda = accuracy_score(y_test, y_pred_Ida) #Calculate accuracy
...
iris = load iris() #Load the Iris dataset into the variable iris
...
X_scaled= scaler.fit_transform(X) # Standardize the features and store them in X scaled
#Step 3: Perform PСА
...
print("\nExplained Variance Ratio:", pca.explained_variance_ratio ) #Print the explained variance ratio of each principal component
...
kmeans.fit(X pca) # Fit the KMeans algorithm on the PCA-reduced data
y_kmeans = kmeans.predict(X_pca) # Predict the cluster for each data point and store the cluster labels in y kmeans
...
for i, size in enumerate(np.bincount(y kmeans)): # Iterate over the sizes of each cluster
...
from sklearn.metrics import confusion_matrix, accuracy_score, precision_score, recall_score, fl_score # Import various metrics to evaluate the model
#Load the dataset
...
X_scaled = scaler.fit_transform(X) # Fit the scaler to the feature data and transform it, standardizing the features
#Split the data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size-0.3, random_state=42) # Split the data into training and testing sets with 30% of the data used for testing and a fixed random seed for reproducibility
# Initialize and train the SVM model with balanced class weights
model = SVC(kernel=rbf, gamma='scale', C=1.0, class_weight='balanced', probability=True) # Create an instance of SVC with radial basis function kernel, automatic gamma scaling, regularization parameter C set to 1.0, balanced class weights, and probability estimates enabled
model.fit(✗ train, y_train) #Train the SVM model using the training data
...
print("\nConfusion Matrix:")
cm = confusion_matrix(y_test, y_pred) # Compute the confusion matrix to evaluate the performance of the classification
print(cm) # Print the confusion matrix
precision = precision_score(y_test, y_pred, average=None) #Calculate precision scores for each class, without averaging
recall = recall_score(y_test, y_pred, average=None) # Calculate recall scores for each class, without averaging
fl = fl_score(y_test, y_pred, average=None) # Calculate F1 scores for each class, without averaging
